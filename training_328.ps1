...
if ($asr -eq "ave") {
    ...
    python syncnet_328.py --save_dir ./syncnet_ckpt/$person_name --dataset_dir $data_dir --asr $asr --epochs 150
    ...
    python train_328.py --dataset_dir $data_dir --save_dir ./checkpoint/$person_name --asr $asr --use_syncnet --syncnet_checkpoint "$syncnet_path" --batchsize 8 --epochs 100
} else {
    # 3. 训练主模型 (Hubert 模式，不需要 SyncNet)
    Write-Host "[STEP 3] Training main model on all sub-datasets..." -ForegroundColor Cyan
    python train_328.py --dataset_dir $data_dir --save_dir ./checkpoint/$person_name --asr $asr --batchsize 8 --epochs 100
}
