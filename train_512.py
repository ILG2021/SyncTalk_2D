...
from datasetsss_512 import MyDataset
from syncnet_512 import SyncNet_color
...
def get_args():
    parser = argparse.ArgumentParser(description='Train 512',
...
def train(net, epoch, batch_size, lr):
    ...
    train_dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, drop_last=False, num_workers=0)
    ...
    for e in range(epoch):
        ...
        if args.see_res:
            net.eval()
            img_concat_T, img_real_T, audio_feat = dataset.__getitem__(random.randint(0, dataset.__len__()-1))
            ...
            cv2.imwrite("./train_tmp_img/epoch_"+str(e)+"_512.jpg", pred)
            cv2.imwrite("./train_tmp_img/epoch_"+str(e)+"_real_512.jpg", img_real)
